...
with open('subtask2-heterographic-test.gold', 'r') as f:
    lines = f.readlines()
    #for each line, append the line to the list
    for line in lines:
        line = line.strip()
        line = line.split('\t')
        het_pun_loc_dict[line[0]] = line[1]
#close the file
f.close()

#reading .gold pun sense file
with open('subtask3-heterographic-test.gold', 'r') as f:
    lines = f.readlines()
    #for each line, append the line to the list
    for line in lines:
        line = line.strip()
        line = line.split('\t')
        het_pun_sense_dict[line[0]] = (line[1:])
#close the file
f.close()
...
for text in root.findall('./text'):
    ...
    # get the text id
    text_id = text.get('id')
    if text_id not in het_pun_loc_dict: #sentence does not contain a pun
        #add the sentence to the list of negative samples
        neg = 1
    else:
        pun_loc = het_pun_loc_dict[text_id]
        #use .get to get the pun sense, if it exists, else use the default value
        pun_sense = het_pun_sense_dict.get(pun_loc, ['NA'])
...
with open('het_pun_sentence_list.txt', 'w') as f:
    f.truncate(0)
...
with open('het_pun_list.txt', 'w') as f:
    f.truncate(0)
...
with open('het_neg_samples.txt', 'w') as f:
    f.truncate(0)
